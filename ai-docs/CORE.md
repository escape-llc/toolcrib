...
> **Content-Security-Policy note.** The two halves of toolcrib's styling mechanism have different CSP stories under a strict `style-src` (no `'unsafe-inline'`). Inline `style` objects — the vast majority of every component's styling — are likely fine as-is: React applies the `style` prop via direct CSSOM property assignment (`element.style.setProperty(...)`/`element.style[prop] = value`), never by writing a literal `style="..."` attribute string, and CSP's `style-src-attr` enforcement specifically hooks attribute mutation, not CSSOM property calls. The real gap is the handful of dynamically-injected `<style>` *tags* (the typography base rule, responsive `@media` blocks, a few hover rules, all via `injectGlobalStyle`/`upsertGlobalStyle`) — genuinely subject to `style-src-elem`, and the current implementation sets no nonce on the elements it creates, so a nonce-based policy would block them until that's added. Everything else about toolcrib is PWA/offline-friendly: zero runtime `fetch`/network calls anywhere in the vendored source, and `localStorage` usage (saved Theme Editor presets) is guarded to degrade safely rather than throw when storage is unavailable.
