...
git add package.json package-lock.json && git commit -m "Bump toolcrib CLI to vX.Y.Z"
...
git tag cli-vX.Y.Z && git push origin cli-vX.Y.Z   # tag *after* publish succeeds, so a failed/declined publish never leaves a tag pointing at content that was never actually shipped
```
