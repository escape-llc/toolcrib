 ...
 * its own extra `<div>`/`<span>` (`Modal`, `Popup`, `SlideOut`). Extracted
 ...
 * way `Modal`'s and `SlideOut`'s copies correctly did — reported directly.
 *
 ...
 * `Modal`/`Popup`/`SlideOut` can't do the same, since each also needs a
