...
| `p-2 md:p-4 lg:p-6` (density that changes per breakpoint) | A `{ base, sm?, md?, lg?, xl? }` object on `paddingMode`/`marginMode`/`cornerRadiusMode`, set once on `<ThemeProvider initialParameters={{...}}>` — see `CORE.md`'s Core Principles, principle 8. One global setting every component responds to at once, not a per-element class you repeat on each one. |
