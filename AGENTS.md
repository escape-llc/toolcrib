...
1. **Bump the root `package.json` `"version"` field only** — never `cli/package.json`'s. The CLI is an independently-versioned, independently-published npm package (see its own section above); its version has no relationship to the toolkit's. A toolkit release doesn't touch the CLI's version at all.
...
4. **Commit and push to `main` first, tag second** — the tag has to point at a commit where `package.json`'s version already matches what's about to be tagged, or `release.yml`'s own "Verify tag matches package.json version" step fails the whole workflow.
5. **Tag and push**: `git tag -a vX.Y.Z -m "vX.Y.Z" && git push origin vX.Y.Z`. This is what actually triggers the release — a public, visible action (a new GitHub Release, downloadable by anyone) — so this step specifically needs the maintainer's go-ahead, not just steps 1-4.
6. **Watch it through, don't assume it succeeded**: `gh run watch <run-id> --exit-status` (find the run via `gh run list --workflow=release.yml --limit 1`) blocks until the workflow finishes and surfaces a real failure instead of a silently abandoned release. Confirm the release actually exists after: `gh release view vX.Y.Z`.

This whole sequence is a maintainer-initiated action end to end — steps 1-4 are safe/reversible (local commits, nothing public yet), but step 5 is not (a pushed tag triggers a real public release immediately, no draft/review stage in between) and should never happen without the maintainer having explicitly asked for that specific version number first.
