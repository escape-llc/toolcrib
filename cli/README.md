...
Bootstraps and maintains the [toolcrib](https://github.com/escape-llc/toolcrib)
UI toolkit inside your own project. Downloads a release, computes what's
changed, and stages every addition/modification as a reviewable patch — it
never writes to a pre-existing file directly. You (or your AI assistant)
apply what you want with `toolcrib apply`.
...
Or run without installing, via `npx toolcrib <command>`.

## Commands

| Command | What it does |
|---|---|
| `toolcrib init [--version <x.y.z>]` | Stage installation of the toolkit into the current project. Defaults to the latest release. |
| `toolcrib merge [--version <x.y.z>]` | Stage an update from the currently installed version to another. |
| `toolcrib apply` | Apply every staged patch in `./toolcrib-patches/`, then delete the folder regardless of outcome. |
| `toolcrib doctor` | Read-only check for local drift and available updates. |
| `toolcrib versions` | List available toolkit releases. |

Nothing is ever written to an existing file outside of `toolcrib apply` —
`init`/`merge` only ever produce patches for you to review first.
...
npm pack --dry-run   # confirm exactly what would ship — should be
                      # package.json + src/ only, ~15 files
npm publish
```
...
else in this folder (tests, the integration-test mock server, this
README, `package-lock.json`) is left out even though it's sitting right
here in the repo. Without it, npm defaults to "include everything not
...
complete, correct set — `package.json` and the `bin` entry are always
included automatically regardless of what's listed.
