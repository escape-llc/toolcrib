#!/usr/bin/env node
/**
 * Packages ./dist into toolcrib.zip plus a toolcrib.zip.sha256 sidecar.
 * The checksum guards against transport corruption (a truncated/flaky
 * download), not a compromised source — see design notes on that
 * distinction. Kept as a separate step from build-release.js so
...
function main() {
  if (!fs.existsSync(path.join(DIST, 'toolcrib.config.json'))) {
    throw new Error('./dist/toolcrib.config.json not found — run build-release.js first.');
  }

  fs.rmSync(ZIP_PATH, { force: true });

  // Zip the *contents* of dist/, not the dist/ folder itself, so the
